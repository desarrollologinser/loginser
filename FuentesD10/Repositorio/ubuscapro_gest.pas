...
dm.busqueda_gest  //TbFIBDataset en udm
dm.dsbusqueda_gest  //TDataSource en udm linked with dm.busqueda_gest y linked con RxDbGrid1
...
with fbuscapro do begin
 ...
 for i:=0 to fbuscapro.dbgrid1.selectedrows.count-1 do begin
   dm.busqueda_gest.bookmark:=fbuscapro.dbgrid1.selectedrows.items[i];
        dm.q.close;
        dm.q.sql.clear;
        dm.q.sql.add('select cod_ubic from ubicart_a where cod_art='+quotedstr(dm.busqueda_gest.fieldbyname('cod_art').asstring));
        dm.q.open;
        if dm.q.isempty then begin
                dm.ubicart.append;
                dm.ubicart.fieldbyname('cod_art').asstring:=dm.busqueda_gest.fieldbyname('cod_art').asstring;
...
uses
  ...
  StdCtrls, Buttons,Grids, db,ComCtrls,ExtCtrls,
   DBGrids, JvDBGrid,FIBDataSet,sLabel,
  sSpeedButton, sBevel, sEdit, sGroupBox, Mask, sCustomComboEdit,
  sTooledit, sPanel, sMaskEdit, JvExDBGrids;

const
    max=20;

type
  Tfbuscapro_gest = class(TForm)
    grid: TJvDBGrid;
    pn_1: TsPanel;
    Bevel2: TsBevel;
    Bevel1: TsBevel;
...
var
  fbuscapro_gest: Tfbuscapro_gest;
...
procedure Tfbuscapro_gest.limpia_wide;   //Limpia vector de anchuras al cerrar form
var     i:integer;
begin
for i:=1 to max do wide[i]:=0;
end;

procedure Tfbuscapro_gest.aplica_wide;        //Aplica vector de anchuras al enseñar form
var     i:integer;
begin
for i:=1 to max do             //Si anchura > 0 -> asigna anchura
   if wide[i]>0 then grid.columns[i-1].width:=wide[i];
for i:=0 to grid.columns.count-1 do grid.columns[i].width:=grid.columns[i].width+14;

grid.RowsHeight:=row_height;                         //altura de filas
end;

procedure Tfbuscapro_gest.limpia_valor;      //limpia vector de valores de retorno
var     i:integer;
begin
for i:=1 to max do valor[i]:=-1;
end;

procedure Tfbuscapro_gest.limpia_fields;   //limpia vector de campos
begin
...
procedure Tfbuscapro_gest.limpia_orden;      //limpia vector de orden
var     i:integer;
begin
for i:=1 to max do orden[i]:=0;
end;

procedure Tfbuscapro_gest.sb_buscaClick(Sender: TObject);     //Aplica nuevo filtro
...
dm.busqueda_gest.close;
dm.busqueda_gest.sqls.SelectSQL.clear;
if distinct=1 then dm.busqueda_gest.sqls.SelectSQL.add('select distinct ')
        else dm.busqueda_gest.sqls.SelectSQL.add('select ');
dm.busqueda_gest.sqls.SelectSQL.add(sel+' from '+from+' where (');
if where<>'' then dm.busqueda_gest.sqls.SelectSQL.add(where+' and ');
...
dm.busqueda_gest.sqls.SelectSQL.add(fil+') ');
if group<>'' then dm.busqueda_gest.sqls.SelectSQL.add('group by '+group);
if ord<>'' then dm.busqueda_gest.sqls.SelectSQL.add('order by '+ord);
dm.busqueda_gest.open;
aplica_wide;
asigna_columns;
asigna_fil;
fillimpio:=false;
...
procedure Tfbuscapro_gest.sb_okClick(Sender: TObject);     //Devuelve el valor de la fila en valor[x]
var     i:integer;
begin
 if ((not(dm.busqueda_gest.isempty)) and not(multiselect)) then begin
        limpia_valor;
        resultado:=true;
        i:=0;
        while i<fields.count do begin
                valor[i+1]:=dm.busqueda_gest.fields[i].text;
                inc(i);
        end;
end;
limpia_wide;
...
procedure Tfbuscapro_gest.ed_buscaKeyPress(Sender: TObject; var Key: Char);    //Pulsa el Enter
begin
if ((key=chr(13)) and not(living)) then sb_buscaClick(Sender);
end;

procedure Tfbuscapro_gest.FormShow(Sender: TObject);       //Inicio
var    i:integer;
begin
width:=ancho;
height:=alto;
if multiselect then grid.options:=[dgtitles,dgrowlines,dgindicator,dgMultiSelect]
        else grid.options:=[dgtitles,dgrowlines,dgindicator];
ordenando:=false;
...
dm.busqueda_gest.sqls.SelectSQL.add(sel+' from '+from);
if where<>'' then dm.busqueda_gest.sqls.SelectSQL.add(' where ('+where);
if not(fillimpio) then begin
  if where<>'' then  dm.busqueda_gest.sqls.SelectSQL.add(' and '+fil+')')
  else  dm.busqueda_gest.sqls.SelectSQL.add(' ('+fil+')')
end else if where<>'' then  dm.busqueda_gest.sqls.SelectSQL.add(')');
if group<>'' then dm.busqueda_gest.sqls.SelectSQL.add(' group by '+group);
if ord<>'' then dm.busqueda_gest.sqls.SelectSQL.add(' order by '+ord);
dm.busqueda_gest.open;
asigna_busca_orden;
...
procedure Tfbuscapro_gest.asigna_fil;                 //Asigna Filtro
begin
lb_filtro.caption:=fil_tit;
end;

procedure Tfbuscapro_gest.asigna_columns;           //Asigna titulos de columnas
var     i:integer;
begin
i:=0;
while i<titulos.count do begin
   grid.columns[i].title.caption:=titulos[i];
   inc(i);
end;
end;

procedure Tfbuscapro_gest.asigna_busca_orden;         //Asigna Busca y Orden
...
o:=o+' '+s;
lb_orden.caption:=o;
if ((dm.busqueda_gest.fields[busca-1] is TFIBIntegerField) or
    (dm.busqueda_gest.fields[busca-1] is TFloatField) or
    (dm.busqueda_gest.fields[busca-1] is TBCDField) or
    (dm.busqueda_gest.fields[busca-1] is TSmallintField) or
    (dm.busqueda_gest.fields[busca-1] is TIntegerField)) then begin
                                                        tip_field:=1;
                                                        rg_int.visible:=true;
...
if ((dm.busqueda_gest.fields[busca-1] is TFIBStringField) or
    (dm.busqueda_gest.fields[busca-1] is TStringField)) then begin
                                                        tip_field:=0;
                                                        rg_str.visible:=true;
...
if ((dm.busqueda_gest.fields[busca-1] is TDateField) or
    (dm.busqueda_gest.fields[busca-1] is TDateTimeField)) then begin
                                                        tip_field:=2;
                                                        rg_str.visible:=false;
                                                        rg_int.visible:=true;
                                                        ...
                                                        if ed_date.date<>0 then
                                                          ed_date.date:=strtodate(dm.busqueda_gest.fields[busca-1].text);
                                                        living:=false;
                                                end;
end;

procedure Tfbuscapro_gest.sb_SpeedButton3Click(Sender: TObject);   //Cancela
begin
limpia_valor;
limpia_wide;
close;
...
procedure Tfbuscapro_gest.FormClose(Sender: TObject; var Action: TCloseAction);   //Cierra Form
begin
fields.free;
titulos.free;
end;

procedure Tfbuscapro_gest.FormCreate(Sender: TObject);
begin
grid.datasource:=dm.dsbusqueda_gest;
end;

procedure Tfbuscapro_gest.sb_SpeedButton4Click(Sender: TObject);    //Limpia Filtro
begin
dm.busqueda_gest.close;
dm.busqueda_gest.sqls.SelectSQL.clear;
if distinct=1 then dm.busqueda_gest.sqls.SelectSQL.add('select distinct ')
...
procedure Tfbuscapro_gest.sb_SpeedButton5Click(Sender: TObject);       //Limpia Orden
begin
if not(ordlimpio) then begin
        limpia_orden;
        ord:='';
        dm.busqueda_gest.close;
        dm.busqueda_gest.sqls.SelectSQL.Delete(dm.busqueda_gest.sqls.SelectSQL.Count-1);
        dm.busqueda_gest.open;
        aplica_wide;
        asigna_columns;
        asigna_busca_orden;
...
procedure Tfbuscapro_gest.ed_buscaKeyUp(Sender: TObject; var Key: Word;          //Pulsa tecla
  Shift: TShiftState);
...
procedure Tfbuscapro_gest.gridCellClick(Column: TColumn);
begin
ordenando:=false;
end;

procedure Tfbuscapro_gest.gridDblClick(Sender: TObject);
begin
if not(ordenando) then sb_okclick(self);
end;

procedure Tfbuscapro_gest.gridTitleBtnClick(Sender: TObject; ACol: Integer;
  Field: TField);
...
dm.busqueda_gest.close;
if not(ordlimpio) then dm.busqueda_gest.sqls.SelectSQL.Delete(dm.busqueda_gest.sqls.SelectSQL.Count-1);
if group<>'' then dm.busqueda_gest.sqls.SelectSQL.add(' group by '+group);
if ord<>'' then dm.busqueda_gest.sqls.SelectSQL.add(' order by '+ord);
dm.busqueda_gest.open;
aplica_wide;
...
procedure Tfbuscapro_gest.gridGetBtnParams(Sender: TObject; Field: TField;
  AFont: TFont; var Background: TColor; var SortMarker: TSortMarker;
  IsDown: Boolean);
...
procedure Tfbuscapro_gest.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=VK_ESCAPE then sb_SpeedButton3Click(self);
  if Key=VK_F1 then sb_okClick(self);
  if Key=VK_UP then dm.busqueda_gest.prior;
  if Key=VK_DOWN then dm.busqueda_gest.next;
end;
