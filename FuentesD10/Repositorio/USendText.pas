...
uses u_c_ontinyent, IdGlobal;

destructor ThSendTextHilo.destroy;
begin
  if launcher=0 then u_c_ontinyent.hiloactivo:=false;
  inherited Destroy;
end;
...
begin
     ...
     try
        ...
        if launcher=0 then u_c_ontinyent.htmldevuelto:=IdHTTP.Post(Cadena2URL(surl+snomphp),data);
        Application.ProcessMessages;
        data.Free;
        IdHTTP.Free;
     except
