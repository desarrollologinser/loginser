...
function StreamHash(Stream: TStream; Algorithm: THashAlgorithm): string;
function FileHash(Archivo: string; Algorithm: THashAlgorithm): string;
function StringHash(Str: string; Algorithm: THashAlgorithm): string;
...
begin
  ...
  try
    ...
    try
      Result:= StreamHash(Stream,Algorithm);
    finally
      Stream.Free;
    end;
  except end;
